...
if (isset($_POST['nombre'])) {
    ...
    if (!empty($nombre)) {
        ...
        try {
            ...
            // 1. Buscar el ID más alto actual en la tabla proveedores
            $sqlMax = "SELECT MAX(id) AS max_id FROM proveedores";
            ...
            // 2. Si hay registros se suma 1
            $nuevoId = ($row['max_id'] !== null)
                        ? $row['max_id'] + 1
                        : 1;

            // 3. Insertar proveedor
            $sql = "INSERT INTO proveedores (id, nombre)
                    VALUES (:id, :nombre)";
